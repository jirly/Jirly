define(["app"], function (app) {
    ...
    var voicePhraseIndexCtrl = function ($scope, NgTableParams, repo, util, $location) {
        var phraseConf = {
        ...
        $scope.params = {};
        $scope.identifyCompleteUrl = xpath.service('phrase/voice/identifycomplete/');;
        /**
        ...
        //新增
        $scope.add = function() {
            $location.path("/appmanage/voicephrase/edit/add").replace();
        }
        ...
        $scope.edit = function(phrase) {
            $location.path("/appmanage/voicephrase/edit/"+phrase.id).replace();
        };
    };

    voicePhraseIndexCtrl.$inject = injectParams;
    app.register.controller('voicePhraseIndexCtrl', voicePhraseIndexCtrl);
});
